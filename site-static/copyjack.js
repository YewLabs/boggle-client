...
let copyJacked = false;
const copyJack = (copyable, skips) => {
  const f = (t, x, y) =>
    Array.from(t.getElementsByTagName(x)).forEach(
      (e) => Array.from(e.classList).includes("copy-verbatim") || y(e)
    );
  const g = (t, x, y) =>
    Array.from(t.getElementsByClassName(x)).forEach(
      (e) => Array.from(e.classList).includes("copy-verbatim") || y(e)
    );
  // do not copyjack twice
  if (copyJacked) return;
  copyJacked = true;
...
const copyContents = async (btnElt, btnText) => {
  const copyable = document.getElementsByClassName("puzzle")[0];
  if (copyable) {
